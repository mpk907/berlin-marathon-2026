...
function longRunInsight(weeklyData, trainingPlan, currentWeek) {
  ...
  return {
    ...
    body: `Plan: ${plannedKm} km, but your longest is ${longestEver.toFixed(1)} km — that's a +${Math.round((plannedKm / longestEver - 1) * 100)} % jump. Aim for ~${recommended} km this Sunday (+10 % vs longest). The next 5 km comes from consistency, not single-week leaps.`,
  };
}
  ...
  if (lastQuality && nextQuality) {
    ...
    if (ratio > 1.4) {
      ...
      return {
        ...
        body: `Last quality: ${lastKm.toFixed(1)} km @ ${lastQuality.pace || "—"}/km, avg HR ${lastQuality.avgHr}. Plan W${nextQuality.week} ${nextQuality.day}: ${nextKm} km ${nextQuality.type} — that's a +${Math.round((ratio - 1) * 100)} % jump. Try ${progressed} km of work next time (+25 %). Advance one variable per cycle: more reps, longer reps, or faster pace — not all at once.`,
      };
    }
    return {
      icon: "💪",
